...
def _format_gps(gps_str):
    """Ensure GPS values always have 6+ decimal places (ONDC requirement)."""
    try:
        parts = (gps_str or "0.0,0.0").split(",")
        if len(parts) == 2:
            return f"{float(parts[0].strip()):.6f},{float(parts[1].strip()):.6f}"
...
def process_confirm(data, log_name=None):
    ...
    try:
        ...
        # Store original timestamps from confirm request (FIX 4H — echo back in on_status)
        now_iso = datetime.utcnow().isoformat() + "Z"
        order.confirm_created_at = order_data.get("created_at") or context.get("timestamp") or now_iso
        billing = order_data.get("billing", {})
        order.billing_created_at = billing.get("created_at") or order.confirm_created_at
        ...
        # Billing tax number
        order.billing_tax_number = billing.get("tax_number") or ""
        ...
        # Store quote data for on_status echoing
        quote = order_data.get("quote")
        if quote:
            order.quote_data = json.dumps(quote)
...
def process_status(data, log_name=None):
    ...
    try:
        ...
        for item in order.items:
            ...
            grand_total += item_price * int(item.quantity or 1)
        ...
        store_gps = _format_gps(settings.get("store_gps") or "12.9716,77.5946")
        store_name = settings.get("store_name") or settings.legal_entity_name or "Store"
        store_locality = settings.get("store_locality") or ""
        store_city = settings.get("store_city_name") or settings.city or ""
        store_state = settings.get("store_state") or ""
        store_area_code = settings.get("store_area_code") or ""
        location_id = f"LOC-{settings.city}"

        # Shipping GPS from order
        shipping_gps = _format_gps(order.get("shipping_gps") or "0.0,0.0")
        shipping_address = {}
        try:
            shipping_address = json.loads(order.get("shipping_address") or "{}")
        ...
        fulfillment = {
            ...
            "start": {
                ...
                "time": {
                    "range": {
                        "start": order.get("confirm_created_at") or order.creation.isoformat() + "Z" if order.creation else now_iso,
                        "end": now_iso,
                    },
                    "timestamp": now_iso,
                },
        ...
        if order.get("tracking_url"):
            ...
            fulfillment["@ondc/org/tracking_url"] = order.tracking_url

        # Use stored created_at from original confirm, or fallback to order creation time
        order_created_at = order.get("confirm_created_at") or (order.creation.isoformat() + "Z" if order.creation else now_iso)
        billing_created_at = order.get("billing_created_at") or order_created_at

        order_payload = {
            "id": order.ondc_order_id,
            "state": order.order_status or "Accepted",
            "provider": {
            ...
            "billing": {
                ...
                "email": order.customer_email or "",
                "phone": order.customer_phone or "",
                "tax_number": order.get("billing_tax_number") or settings.get("gst_number") or "29AABCU9603R1ZM",
                "created_at": billing_created_at,
                "updated_at": now_iso,
            },
            "fulfillments": [fulfillment],
            "quote": order.get("quote_data") and json.loads(order.get("quote_data")) or {
                ...
                "@ondc/org/settlement_details": [
                    {
                        ...
                        "beneficiary_name": settings.get("legal_entity_name") or "Seller",
        ...
        # Get bap_uri from context (works for both solicited and unsolicited callbacks)
        bap_uri = data.get("context", {}).get("bap_uri") or order.get("bap_uri")
...
def auto_progress_fulfillment(order_id, transaction_id, bap_uri, bap_id, context_data):
    """
    Auto-progress fulfillment states after order confirmation (FIX 5).
    Sends unsolicited on_status callbacks for each state transition.
    ...
    for state in states:
        ...
        try:
            ...
            # Build FULL context for unsolicited on_status callback
            context = {
                "domain": context_data.get("domain", "ONDC:RET10") if context_data else "ONDC:RET10",
                "country": context_data.get("country", "IND") if context_data else "IND",
                "city": context_data.get("city", settings.city) if context_data else settings.city,
                "action": "on_status",
                "core_version": context_data.get("core_version", "1.2.0") if context_data else "1.2.0",
                "bap_id": (context_data.get("bap_id") if context_data else None) or bap_id,
                "bap_uri": (context_data.get("bap_uri") if context_data else None) or bap_uri,
                "bpp_id": settings.subscriber_id,
                "bpp_uri": settings.subscriber_url,
                "transaction_id": transaction_id,
                "message_id": str(uuid.uuid4()),  # Unique per callback
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "ttl": "PT30S",
            }

            # Call process_status with proper context
            status_data = {
                "context": context,
            ...
            frappe.log_error(
                f"Auto-progress {order_id} state={state}: {str(e)}\n{traceback.format_exc()}",
                "ONDC Auto Progress"
            )
...
def process_cancel(data, log_name=None):
    ...
    try:
        ...
        payment_type_map = {"Prepaid": "ON-ORDER", "COD": "ON-FULFILLMENT", "Credit": "POST-FULFILLMENT"}
        ondc_payment_type = payment_type_map.get(order.payment_type, "ON-ORDER")

        # Build items array from order items child table
        items_list = []
        quote_breakup = []
        item_total = 0.0
        for item in order.items:
            items_list.append({
                "id": item.ondc_item_id,
                "fulfillment_id": order.fulfillment_id or "F1",
                "quantity": {"count": int(item.quantity or 1)},
            })
            line_total = float(item.amount or item.price or 0)
            item_total += line_total
            quote_breakup.append({
                "title": item.item_name or item.ondc_item_id,
                "@ondc/org/item_id": item.ondc_item_id,
                "@ondc/org/item_quantity": {"count": int(item.quantity or 1)},
        ...
        # Store location for fulfillment start
        store_gps = settings.get("store_gps") or "12.9716,77.5946"
        store_name = settings.get("store_name") or settings.legal_entity_name or "Store"
        store_locality = settings.get("store_locality") or ""
        store_city = settings.get("store_city_name") or settings.city or ""
        store_state = settings.get("store_state") or ""
        ...
        # Build cancellation response with full order echo
        order_payload = {
            "id": order.ondc_order_id,
            "state": "Cancelled",
            "provider": {
                    ...
                    "tags": [
                        {
                            ...
                            "list": [
                                ...
                                {"code": "rto_id", "value": ""},
                                {"code": "reason_id", "value": str(cancellation_reason_id)},
                                {"code": "initiated_by", "value": data.get("context", {}).get("bap_id", "")},
                            ],
                        }
                    ],
                }
            ...
            "quote": {
                "price": {"currency": "INR", "value": str(round(item_total, 2))},
                "breakup": quote_breakup,
                "ttl": "PT15M",
            },
            "payment": {
                "type": ondc_payment_type,
                "collected_by": "BAP" if ondc_payment_type == "ON-ORDER" else "BPP",
                "status": "NOT-PAID",
                "@ondc/org/buyer_app_finder_fee_type": "percent",
                "@ondc/org/buyer_app_finder_fee_amount": str(settings.get("buyer_finder_fee") or "3"),
                "@ondc/org/settlement_basis": "delivery",
                ...
                "@ondc/org/settlement_details": [
                    {
                        ...
                        "settlement_amount": str(round(float(order.total_amount or 0), 2)),
                    }
                ],
            },
            "tags": [
            ...
            "created_at": order.creation.isoformat() + "Z" if order.creation else "",
